...
public class QuestController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        InventoryController1.Instance.OnInventoryChanged += CheckInventoryForQuests;
    }
    ...
    {
        Dictionary<int, int> itemCounts = InventoryController1.Instance.GetItemCounts();

        foreach(QuestProgress quest in activateQuests)
        {
            foreach(QuestObjective questObjective in quest.objectives)
            {
                if (questObjective.type != ObjectiveType.ItemCollection) continue;
    ...
    public bool RemoveRequiredItemsFromInventory(string questID)
    {
        ...
        {
            if(objective.type == ObjectiveType.ItemCollection && int.TryParse(objective.objectiveID, out int itemID))
            {
        ...
        {
            InventoryController1.Instance.RemoveItemsFromInventory(itemRequirement.Key, itemRequirement.Value);
        }
